...
# From mitou/jr.mitou.org, _tasks/convert_ja2en_by_llm.rb, the script that
# turns the year's Japanese project titles and abstracts into English for the
# site's project pages. The repository's model, gpt-5-chat-latest, is not in
# the catalog, so gpt-5 is substituted; the commented out alternate models
# are dropped so one model string is left. The same params hash is sent twice
# per project, once for the title and once for the abstract.
