// From Actual-Chat/actual-chat,
// src/dotnet/Transcription.Service/Transcribers/OpenAITranscriber.cs. The audio
// client is built once from Options.Model, which the repository pins to a
// transcription id the catalog does not price, so the cheapest catalog id from
// the same vendor is substituted as DefaultModel; nothing else about the call is
// changed. The Ogg conversion and the word time map after the call are trimmed.
...
public sealed class OpenAITranscriber : IOfflineTranscriber
{
    ...
    public TranscriberInfo Info { get; } = new() {
        ...
        // The model takes a free-form prompt, and the duration is known here, so the budget
