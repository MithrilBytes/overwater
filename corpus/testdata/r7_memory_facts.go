// From angelnicolasc/graymatter: pkg/memory/extract.go, with the
// ConsolidateModel default from config.go, which is where the repository
// keeps it; the model reaches the call through the ExtractConfig
// interface. The Ollama consolidation branch is dropped.
...
// Config is the subset of graymatter.Config the extractor reads.
type Config struct {
	// AnthropicAPIKey enables Anthropic-backed consolidation and extraction.
