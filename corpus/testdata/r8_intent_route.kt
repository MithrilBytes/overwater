// From jinia91/blog,
// service/ai-second-brain/ai-second-brain-core/src/main/kotlin/kr/co/jiniaslog/ai/domain/agent/IntentRouterAgent.kt,
// with the lightweightChatClient bean from service/AiConfig.kt and the
// Intent enum from domain/agent/Intent.kt in the same module. The Gemini
// model id is configured for Spring AI's google-genai starter outside the
// tree, so the catalog id gemini-2.5-flash is set on the bean here. The
// variant prompt that prepends recent conversation history is dropped; the
// prompt is Korean in the original and is kept that way.
...
의도:"""
     ...
     */
    fun classifyWithSubIntents(message: String): Pair<Intent, List<Intent>> {
        val prompt = ROUTER_PROMPT.format(message)
    ...
    fun classify(message: String): Intent {
        return classifyWithSubIntents(message).first
    }
