...
func README(src string, f Facts) (string, error) {
	...
	// The marker is inside the generated span on purpose: it is what an
	// editor sees in the source, and regenerating restores it.
	list := "<!-- Generated by tools/sync-docs. Add the rule, then run it. -->\n" +
		wrap(f.Rules, 72)
	out = rulesSectionRe.ReplaceAllString(out, "${1}"+list+"${2}")
	return out, nil
}
